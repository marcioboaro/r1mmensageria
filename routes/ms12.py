...
def ms12(ms12: MS12, public_id=Depends(auth_handler.auth_wrapper)):
    try:
        ...
        if ms12.ID_Transacao_Unica is not None:
            ...
            if conn.execute(command_sql).fetchone() is None:
                return {"status_code": 422, "detail": "M010001 - ID_Transacao_Unica não Existe"}

        # validando Codigo_Conformidade_POD
        if ms12.Codigo_Conformidade_POD is None:
            return {"status_code": 422, "detail": "M010008 - Codigo_Conformidade_POD obrigatório"}
        ...
        return {"status_code": 200, "detail": "M06023 - Enviado com sucesso"}
