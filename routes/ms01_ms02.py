...
#@ms01_ms02.post("/ms01", tags=["ms02"], response_model=MS04, description="Consulta catalogo Lockers")
@ms01_ms02.post("/ms01", tags=["ms02"], description="Consulta catalogo Lockers")
def ms01(ms01: MS01, public_id=Depends(auth_handler.auth_wrapper)):
    ms01.Codigo_de_MSG = "MS01"
    if ms01.ID_de_Referencia is None:
        raise HTTPException(status_code=422, detail="M02005 - ID_de_Referencia obrigatório")
    if ms01.ID_do_Solicitante is None:
        raise HTTPException(status_code=422, detail="M02006 - ID_do_Solicitante obrigatório")
    if len(ms01.ID_do_Solicitante) != 14:
    ...
    if ms01.ID_Rede_Lockers is not None:
        ...
        if conn.execute(command_sql).fetchone() is None:
            raise HTTPException(status_code=422, detail="M02010 - ID_Rede_Lockers inválido")
