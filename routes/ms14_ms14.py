...
def ms05(ms05: MS05, public_id=Depends(auth_handler.auth_wrapper)):
    try:
        ...
        if record_Porta is None:
            ...
            ms06['Tipo_de_Servico_Reserva'] = ms05.Tipo_de_Serviço_Reserva # Contratação de Serviço de Reserva Com encomendas em Lockers Inteligentes
            ...
            insert_ms05_encomendas(idTransacaoUnica, encomenda, ms05, etiqueta)
            insert_ms05(ms05, idTransacaoUnica, Inicio_reserva, Final_reserva, record_Porta)
            insert_tracking(ms05, idTransacaoUnica, idTicketOcorrencia, encomenda)
...
def insert_tracking(ms05, idTransacaoUnica, idTicketOcorrencia, encomenda):
    try:
        command_sql = f'''INSERT INTO `rede1minuto`.`tracking_encomenda`
                                    ...
                                    VALUES
                                        ...
                                        '{'Reserva efetivada'}',  
                                        {ms05.Tipo_de_Serviço_Reserva});'''  # 1 - Reserva efetivada, 2 - Reserva cancelada, 3 - Reserva em andamento, 4 - Reserva em espera
        command_sql = command_sql.replace("'None'", "Null")
        command_sql = command_sql.replace("None", "Null")
        conn.execute(command_sql)
    except:
